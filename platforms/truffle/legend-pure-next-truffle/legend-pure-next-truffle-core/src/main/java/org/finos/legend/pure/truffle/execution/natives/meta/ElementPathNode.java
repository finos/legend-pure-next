...
public final class ElementPathNode extends PureNode
{
    ...
    @TruffleBoundary
    private static Object buildPath(Object element, MetadataAccess resolver)
    {
        // PackageableElement-ness is established by carrying a package + name
        // pair; collectAncestors() reads via _Any which gracefully no-ops
    ...
    @TruffleBoundary
    private static void collectAncestors(Object pe, List<Object> path, MetadataAccess resolver)
    {
        Object parent = _Any.readBySlot(pe, SLOT_PACKAGE);
        if (parent != null)
        {
            collectAncestors(parent, path, resolver);
        }
        Object nameObj = _Any.readBySlot(pe, SLOT_NAME);
