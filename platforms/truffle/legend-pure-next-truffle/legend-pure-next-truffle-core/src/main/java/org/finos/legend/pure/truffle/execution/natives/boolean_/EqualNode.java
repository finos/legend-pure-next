...
public final class EqualNode extends PureNode
{
     ...
     * org.finos.legend.pure.truffle.execution.natives.collection.ContainsNode}) without
     * paying for an extra {@link RawLambdaCallNode} dispatch per element.
     */
    public static boolean equalsStatic(Object a, Object b, MetadataAccess resolver)
    {
        if (a == b) return true;
    ...
    @com.oracle.truffle.api.CompilerDirectives.TruffleBoundary
    private static boolean slowEqualsStatic(Object a, Object b, MetadataAccess resolver)
    {
        Object rawA = normalizeForEquals(a);
        Object rawB = normalizeForEquals(b);
    ...
    @com.oracle.truffle.api.CompilerDirectives.TruffleBoundary
    private static boolean callPureEquals(Object a, Object b, MetadataAccess resolver)
    {
        if (a == b)
        {
            return true;
        ...
        // call after warmup).
        boolean aIsEnum = _Any.pureTypeIs(a,
                "meta::pure::metamodel::type::Enum");
        boolean bIsEnum = _Any.pureTypeIs(b,
                "meta::pure::metamodel::type::Enum");
        if (aIsEnum)
        {
            Object ea = a;
            ...
            {
                Object eb = b;
                Object eaName = _Any.readBySlot(ea, SLOT_NAME);
                Object ebName = _Any.readBySlot(eb, SLOT_NAME);
                ...
                // {@link org.finos.legend.pure.truffle.execution.PureContext#classifierGenericType}
                // (NOT the raw slot) so codegen / PDB-loaded enum singletons —
                // whose CGT slot is null on purpose, with the enumeration path
                // resolved lazily via {@link EnumValueSingletons} — also produce
                ...
                org.finos.legend.pure.truffle.execution.PureContext lang = org.finos.legend.pure.truffle.execution.PureLanguage.get(null);
                Object eaCgt = lang != null ? lang.classifierGenericType(ea) : _Any.readBySlot(ea, SLOT_CLASSIFIER_GENERIC_TYPE);
                Object ebCgt = lang != null ? lang.classifierGenericType(eb) : _Any.readBySlot(eb, SLOT_CLASSIFIER_GENERIC_TYPE);
        ...
        {
            Object ebName = _Any.readBySlot(b, SLOT_NAME);
        ...
        String ptA = _Any.pureTypeOf(a);
        String ptB = _Any.pureTypeOf(b);
     ...
     */
    @com.oracle.truffle.api.CompilerDirectives.TruffleBoundary
    private static boolean samePureType(Object a, Object b, MetadataAccess resolver)
    {
        Object cgtA = _Any.readBySlot(a, SLOT_CLASSIFIER_GENERIC_TYPE);
        Object cgtB = _Any.readBySlot(b, SLOT_CLASSIFIER_GENERIC_TYPE);
    ...
    @com.oracle.truffle.api.CompilerDirectives.TruffleBoundary
    private static boolean equalByProperties(Object a, Object b, MetadataAccess resolver)
    {
        ...
        if (keyProps != null && !keyProps.isEmpty())
        {
            ...
            {
                Object va = _Any.read(a, propName);
                Object vb = _Any.read(b, propName);
        ...
        // Top-level PDB elements are cached singletons (via PdbModule),
     ...
     * Type on the resolver — see {@link TypeCache}.
     */
    @com.oracle.truffle.api.CompilerDirectives.TruffleBoundary
    private static java.util.Set<String> collectEqualityKeyProperties(Object obj, MetadataAccess resolver)
    {
        // Fast path: PureClassInfo carries the <<equality.Key>> property
        // names (baked in by PureClassRegistry's class-definition walk) —
        // direct lookup by Pure path, no resolver/Type walk needed.
        String purePath = _Any.pureTypeOf(obj);
        if (purePath != null)
        {
            String[] keys = PureClassRegistry.classInfoFor(purePath, resolver).equalityKeys();
            if (keys != null && keys.length > 0)
            {
                return new java.util.LinkedHashSet<>(java.util.Arrays.asList(keys));
            }
        }
        Object cgt = _Any.readBySlot(obj, SLOT_CLASSIFIER_GENERIC_TYPE);
        if (cgt == null)
        {
            return null;
        }
    ...
    private static Object normalizeForEquals(Object v)
    {
        while (true)
        {
            if (_Any.pureTypeIs(v,
                    "meta::pure::metamodel::valuespecification::AtomicValue"))
            {
                Object inner = _Any.readBySlot(v, SLOT_VALUE);
                if (inner != null)
                {
                    v = inner;
                    continue;
