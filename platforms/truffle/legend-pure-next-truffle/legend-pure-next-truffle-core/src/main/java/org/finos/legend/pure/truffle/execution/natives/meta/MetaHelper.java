...
public final class MetaHelper
{
     ...
     * metamodel objects (Any), and PureSequence.
     */
    public static Object getRawValueType(Object value, MetadataAccess resolver)
    {
        ...
        if (value instanceof org.finos.legend.pure.truffle.execution.types.RawClosure rc)
        {
            return getRawValueType(rc.lambda(), resolver);
        }
        if (org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeOf(value) != null)
        {
            var cgt = org.finos.legend.pure.truffle.execution.PureLanguage.get(null).classifierGenericType(value);
            if (cgt != null)
            {
                return org.finos.legend.pure.truffle.compiler.helper._GenericType.type(cgt);
            }
        }
        ...
        {
            // Detect enum value strings: "package::EnumType.VALUE". The
            // resolved prefix must actually BE an Enumeration — arbitrary
            // string content can contain "::" and dots (e.g. the pdb
            // generator's code-template literals, which embed Pure paths and
            // property accesses); treating any resolvable prefix as an enum
            // type routed such strings through the PDB writer's enum arm and
            // silently truncated them to their after-last-dot tail. Same
            // lesson as the date-format heuristic removed below: runtime
            // typing must not degrade into content guessing.
    ...
    private static Object getCollectionType(int size, java.util.function.IntFunction<Object> getter, MetadataAccess resolver)
    {
        if (size == 0)
        {
            return resolver.getElement("meta::pure::metamodel::type::Nil");
        ...
        Object common = org.finos.legend.pure.truffle.compiler.helper._Type.findCommonType(types, false, resolver);
    ...
    public static Object getRawGenericType(Object value)
    {
        ...
        // classifierGenericType. Detect via _Any.pureTypeOf which works for
        ...
        if (value != null
                && org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeOf(value) != null)
        {
            return org.finos.legend.pure.truffle.execution.PureLanguage.get(null).classifierGenericType(value);
        }
        return null;
    }
     ...
     */
    public static Object getRawGenericType(Object value, MetadataAccess resolver)
    {
        ...
        if (type != null)
        {
            return org.finos.legend.pure.truffle.compiler.helper._GenericType.buildUserDefinedGenericType(type, resolver);
        }
        return null;
    }
}
