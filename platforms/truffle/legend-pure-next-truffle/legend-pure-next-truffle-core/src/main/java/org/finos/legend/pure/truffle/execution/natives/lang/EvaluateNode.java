...
public final class EvaluateNode extends PureNode
{
    ...
    @Child
    private org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode dispatchReader = new org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode();
    ...
    private static Object invokeEvaluate(org.finos.legend.pure.truffle.execution.PureContext evaluator, Object[] values,
                                          org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode propertyReader)
    {
        Object fn = values[0];
        if (fn == null || (fn instanceof org.finos.legend.pure.truffle.execution.types.PureSequence ps && ps.isEmpty()))
        {
    ...
    private static void unwrapListValues(Object listObj, List<Object> out)
    {
        ...
        if (org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeIs(listObj,
                "meta::pure::functions::collection::List"))
        {
            Object vals = org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(listObj, SLOT_VALUES);
            int sz = org.finos.legend.pure.truffle.execution.natives.collection.CollectionHelper.size(vals);
            if (sz == 0)
            {
                out.add(org.finos.legend.pure.truffle.execution.types.PureSequence.EMPTY);
            }
            else if (sz == 1)
            {
                out.add(org.finos.legend.pure.truffle.execution.natives.collection.CollectionHelper.at(vals, 0));
            }
            else
            {
                ...
                for (int i = 0; i < sz; i++)
                {
                    arr[i] = org.finos.legend.pure.truffle.execution.natives.collection.CollectionHelper.at(vals, i);
                }
                out.add(new org.finos.legend.pure.truffle.execution.types.ObjectSequence(arr));
            }
        }
        else
        {
