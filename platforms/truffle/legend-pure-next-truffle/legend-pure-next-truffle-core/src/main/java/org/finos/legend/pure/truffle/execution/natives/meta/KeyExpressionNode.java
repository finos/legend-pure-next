...
public final class KeyExpressionNode extends PureNode
{
    ...
    {
        Object keyExpr = org.finos.legend.pure.truffle.execution.TruffleInstanceFactory.createInstance(
                "meta::pure::functions::lang::KeyExpression", resolver);
        org.finos.legend.pure.truffle.compiler.helper._Any.write(keyExpr, "name", values[0]);
        Object expr = values[1];
        if (expr instanceof org.finos.legend.pure.truffle.execution.types.PureSequence)
        {
            org.finos.legend.pure.truffle.compiler.helper._Any.write(keyExpr, "expression", expr);
        }
        else
        {
            org.finos.legend.pure.truffle.compiler.helper._Any.write(keyExpr, "expression",
                    new org.finos.legend.pure.truffle.execution.types.ObjectSequence(new Object[]{expr}));
        }
        if (values.length > 2)
        {
            org.finos.legend.pure.truffle.compiler.helper._Any.write(keyExpr, "add", values[2]);
        }
        return keyExpr;
    }
}
