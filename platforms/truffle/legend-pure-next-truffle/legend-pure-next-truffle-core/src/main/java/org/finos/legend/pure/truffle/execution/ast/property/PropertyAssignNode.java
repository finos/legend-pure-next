...
public final class PropertyAssignNode extends Node
{
    ...
    public PropertyAssignNode(String propertyName, PureNode valueExpr, boolean isAdd)
    {
        ...
        this.boundSlot = propertyName.contains(".")
                ? -1
                : org.finos.legend.pure.truffle.compiler.module.PureClassRegistry.globalSlot(propertyName);
    }
     ...
     *  lambdas. */
    private org.finos.legend.pure.truffle.compiler.module.PureClassInfo cachedClassInfo;
    ...
    public Object executeWithValue(Object value, Object target)
    {
                ...
                && boundSlot < pdo.slots.length)
        {
            ...
            Object coerced = propType != null
                    ? org.finos.legend.pure.truffle.execution.PropertyCoercion.coerce(value, propType)
                    : value;
            // writeSlot (not a raw slots[] store): grows the array when the
            // PDO was constructed mid-bootstrap with a stale slot count.
            pdo.writeSlot(boundSlot, coerced);
            return value;
        }
     ...
     *  {@link org.finos.legend.pure.truffle.execution.types.ObjectSequence}. Sizes both
    ...
    private static Object mergeIntoSequence(Object existing, Object value)
    {
        ...
        if (total == 0) return org.finos.legend.pure.truffle.execution.types.PureSequence.EMPTY;
        Object[] merged = new Object[total];
        int idx = 0;
        idx = appendInto(merged, idx, existing, existingSize);
        appendInto(merged, idx, value, valueSize);
        return new org.finos.legend.pure.truffle.execution.types.ObjectSequence(merged);
    }

    private static int sizeOf(Object value)
    {
        if (value == null) return 0;
        if (value instanceof org.finos.legend.pure.truffle.execution.types.PureSequence ps) return ps.size();
        return 1;
    }
