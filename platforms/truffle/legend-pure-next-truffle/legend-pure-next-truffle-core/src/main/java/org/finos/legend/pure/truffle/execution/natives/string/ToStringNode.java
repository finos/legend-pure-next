...
package org.finos.legend.pure.truffle.execution.natives.string;
...
public final class ToStringNode extends PureNode
{
    ...
    private static final int SLOT_FIRST = org.finos.legend.pure.truffle.compiler.module.PureClassRegistry.globalSlot("first");
    private static final int SLOT_NAME = org.finos.legend.pure.truffle.compiler.module.PureClassRegistry.globalSlot("name");
    private static final int SLOT_SECOND = org.finos.legend.pure.truffle.compiler.module.PureClassRegistry.globalSlot("second");
    ...
    @Child
    private org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode toStringReader = new org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode();
    ...
    private static String convert(Object v, org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode reader)
    {
        if (v == null)
        {
            return "";
    ...
    static String enumQualifiedPath(Object v)
    {
        ...
        String pureType = org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeOf(v);
        ...
        {
            Object name = org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(v, SLOT_NAME);
            if (name instanceof String s)
            {
                Object cgt = org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(v, SLOT_CLASSIFIER_GENERIC_TYPE);
                if (cgt != null)
                {
                    Object enumType = org.finos.legend.pure.truffle.compiler.helper._GenericType.type(cgt);
                    if (enumType != null)
                    {
                        var resolver = org.finos.legend.pure.truffle.execution.PureLanguage.get(null).resolver();
                        String enumPath = org.finos.legend.pure.truffle.compiler.helper._PackageableElement.path(enumType, resolver);
    ...
    @TruffleBoundary
    static String pureToString(Object v, org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode reader)
    {
        if (v == null)
        {
            return "";
        ...
        {
            Object valuesObj = org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(v, SLOT_VALUES);
            if (!(valuesObj instanceof org.finos.legend.pure.truffle.execution.types.PureSequence values) || values.isEmpty())
            {
                return "[]";
            }
        ...
        {
            return "<" + pureToString(org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(v, SLOT_FIRST), reader)
                    + ", " + pureToString(org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(v, SLOT_SECOND), reader) + ">";
        }
        ...
        // Enum value string (format "package::EnumType.VALUE") → return just the
        // value name. The prefix must RESOLVE to an actual Enumeration:
        // ordinary strings can legitimately contain "::" and dots (e.g. the
        // pdb generator's code-template literals embed Pure paths and property
        // accesses), and "looks like" matching made toString LOSSY for them —
        // the self-hosted PDB writer routes plain String atomic values through
        // toString (identity on the JVM/JS platforms) and silently wrote such
        // literals truncated to their after-last-dot tail.
        if (v instanceof String s && s.contains("::") && s.contains("."))
        {
            ...
            if (dotIdx > 0 && dotIdx < s.length() - 1)
            {
                ...
                if (prefix.contains("::"))
                {
                    Object enumType = org.finos.legend.pure.truffle.execution.PureLanguage.get(null).resolver().getElement(prefix);
                    if (enumType != null
                            && org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeIs(enumType,
