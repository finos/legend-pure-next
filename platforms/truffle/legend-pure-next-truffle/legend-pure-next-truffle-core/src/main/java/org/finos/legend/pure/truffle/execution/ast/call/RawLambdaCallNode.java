...
package org.finos.legend.pure.truffle.execution.ast.call;
...
import org.finos.legend.pure.truffle.execution.ast.property.PropertyReadNode;
...
public final class RawLambdaCallNode extends Node
{
    ...
    private org.finos.legend.pure.truffle.execution.PureContext getContext()
    {
        return org.finos.legend.pure.truffle.execution.PureLanguage.get(this);
    }
    ...
    private RootCallTarget getCallTarget(Object v)
    {
        ...
        if (org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeIs(v, LAMBDA_FUNCTION_PATH))
        {
    ...
    private Object dispatchByPureType(Object lambdaOrClosure, Object[] args)
    {
        ...
        if (org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeIs(lambdaOrClosure, LAMBDA_FUNCTION_PATH))
        {
        ...
        if (org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeIs(lambdaOrClosure, PROPERTY_PATH))
        {
            Object[] rawArgs = extractArgs(args);
            if (rawArgs.length > 0)
            {
                return propertyReader.execute(rawArgs[0],
                        (String) org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(lambdaOrClosure, SLOT_NAME));
            }
            return org.finos.legend.pure.truffle.execution.types.PureSequence.EMPTY;
        }
        MetadataAccess resolver =
                getContext().resolver();
        if (org.finos.legend.pure.truffle.compiler.helper._Any.isType(lambdaOrClosure,
                "meta::pure::metamodel::function::FunctionDefinition", resolver))
        {
            Object[] rawArgs = extractArgs(args);
            return getContext().executeFunction(lambdaOrClosure, rawArgs);
        }
        if (org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeIs(lambdaOrClosure,
                "meta::pure::metamodel::function::NativeFunction"))
        {
            Object[] rawArgs = extractArgs(args);
            return org.finos.legend.pure.truffle.execution.natives.lang.EvalNode.dispatch(getContext(), lambdaOrClosure, rawArgs, propertyReader);
        }
