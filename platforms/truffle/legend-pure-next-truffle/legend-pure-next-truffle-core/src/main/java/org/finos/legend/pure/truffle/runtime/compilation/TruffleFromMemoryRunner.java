 ...
 * filtered result as an in-memory {@link InMemoryModule} on top of an
 ...
 * folded by {@link ModuleRegistry#foldChildrenInto foldChildrenInto},
...
public final class TruffleFromMemoryRunner
{
     ...
     * {@link InMemoryModule} named {@code moduleName} with every
            ...
            String moduleName)
    {
        ...
        if (parseDirsFn == null || compileFn == null
                || !_Any.isType(parseDirsFn, "meta::pure::metamodel::function::FunctionDefinition", registry)
                || !_Any.isType(compileFn, "meta::pure::metamodel::function::FunctionDefinition", registry))
        {
        ...
        Object errorsObj = _Any.read(result, "errors");
        ...
        Object elementsObj = _Any.read(result, "elements");
        ...
        registry.register(new InMemoryModule(
                moduleName, deps, testElements, registry));
    }

    private static PureSequence filterTestStereotyped(PureSequence elements, ModuleRegistry registry)
    {
        ...
        for (int i = 0; i < elements.size(); i++)
        {
            ...
            if (el == null) continue;
            if (_Any.pureTypeIs(el, "meta::pure::metamodel::Package") || isTestElement(el, registry))
            {
                kept.add(el);
            }
        }
    ...
    private static boolean isTestElement(Object element, ModuleRegistry registry)
    {
        Object stereos = _Any.read(element, "stereotypes");
