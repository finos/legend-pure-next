...
public final class RegexpExtractNode extends PureNode
{
    ...
    private static boolean asBoolean(Object v)
    {
        ...
        if (v instanceof org.finos.legend.pure.truffle.execution.types.PureSequence s && s.size() == 1) return asBoolean(s.getBoxed(0));
        return false;
    }
    ...
    private static long asLong(Object v)
    {
        ...
        if (v instanceof org.finos.legend.pure.truffle.execution.types.PureSequence s && s.size() == 1) return asLong(s.getBoxed(0));
        return 0L;
    }
    ...
    private static Object extract(String s, String regex, boolean extractAll, int group, int flags)
    {
        ...
        if (!extractAll)
        {
            if (!m.find()) return org.finos.legend.pure.truffle.execution.types.PureSequence.EMPTY;
            String found = m.group(group);
            return found == null ? org.finos.legend.pure.truffle.execution.types.PureSequence.EMPTY : new ObjectSequence(new Object[]{found});
        }
