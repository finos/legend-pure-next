...
public final class NewGenericTypeNode extends PureNode
{
    ...
    {
        if (result == null
                || !org.finos.legend.pure.truffle.compiler.helper._Any.isType(result,
                        "meta::pure::metamodel::type::generics::GenericTypeValue", resolver))
        {
            String pt = org.finos.legend.pure.truffle.compiler.helper._Any.pureTypeOf(result);
        ...
        Object rawType = org.finos.legend.pure.truffle.compiler.helper._GenericType.type(result);
        String classPath = "Unknown";
        if (rawType != null)
        {
            classPath = org.finos.legend.pure.truffle.compiler.helper._PackageableElement.path(rawType, resolver);
            if (classPath == null || classPath.isEmpty())
            {
                Object n = org.finos.legend.pure.truffle.compiler.helper._Any.readBySlot(rawType, SLOT_NAME);
        ...
        {
            if (org.finos.legend.pure.truffle.compiler.helper._GenericType.typeArguments(result) == null
                    || org.finos.legend.pure.truffle.compiler.helper._GenericType.typeArguments(result).isEmpty())
            {
        ...
        Object instance = org.finos.legend.pure.truffle.execution.TruffleInstanceFactory.createInstance(shapeClassPath, resolver);
        if (instance != null)
        {
     ...
     * registered in {@link org.finos.legend.pure.truffle.compiler.module.PureClassRegistry}.
    ...
    private static String resolveShapeFromGeneralizations(Object rawType,
            MetadataAccess resolver)
    {
        Object gens = org.finos.legend.pure.truffle.compiler.helper._Any.read(rawType, "generalizations");
        if (gens instanceof org.finos.legend.pure.truffle.execution.types.PureSequence seq)
        {
            for (int i = 0; i < seq.size(); i++)
            {
                Object gen = seq.getBoxed(i);
                if (gen == null) continue;
                Object general = org.finos.legend.pure.truffle.compiler.helper._Any.read(gen, "general");
                Object generalType = general != null
                        ? org.finos.legend.pure.truffle.compiler.helper._GenericType.type(general)
                        : null;
                if (generalType != null)
                {
                    String ancestorPath = org.finos.legend.pure.truffle.compiler.helper._PackageableElement.path(generalType, resolver);
