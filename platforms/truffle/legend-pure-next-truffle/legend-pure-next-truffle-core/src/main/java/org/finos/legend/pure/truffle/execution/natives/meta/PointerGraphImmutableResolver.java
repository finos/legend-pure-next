...
/**
 * Walks a {@code Map<String, PackageableElement>} graph and produces a deep
 * copy whose pointer slots are rewritten to live targets — the immutable
 * successor to the retired mutating {@code PointerGraphResolver}. The input
 * map and its element PDOs are untouched.
 *
 ...
 *   <li>Registered enum-value singleton ({@link EnumValueSingletons}) → return
 *       as-is. Its type comes from that registration, not a classifierGenericType
 *       slot, so a copy would lose the type and identity-based equality.</li>
 ...
 * </ul>
 *
 * <p>Pointer detection and member-style resolution were carried over from
 * the retired {@code PointerGraphResolver#resolvePointer}.</p>
 */
public final class PointerGraphImmutableResolver
{
     ...
     */
    @TruffleBoundary
    public static List<Object> resolveAllImmutable(Map<String, Object> byPath, MetadataAccess resolver)
    {
    ...
    private static final class Context
    {
        ...
        Context(Map<String, Object> byPath, Set<String> inputPaths, MetadataAccess resolver)
        {
            this.byPath = byPath;
    ...
    private static boolean canonicalisableToDep(PureDynamicObject pdo, String pdoPath, Context ctx)
    {
        ...
        return !_Any.pureTypeIs(pdo, PACKAGE_PATH);
    }
    ...
    private static Object process(Object value, Context ctx)
    {
        ...
        if (value instanceof PureDynamicObject pdo)
        {
            ...
            // Registered enum singletons are typed through EnumValueSingletons, not a CGT slot:
            // copyAndPopulate would yield an untyped Enum PDO (genericType Any, never == the literal).
            // Enum values are immutable, so share them like primitives.
            if (EnumValueSingletons.enumerationPathOf(pdo) != null) return pdo;
            ...
            Object nameObj = _Any.readBySlot(pdo, SLOT_NAME);
        ...
        // Other PureSequence variants (LongSequence, PrependLongSequence)
        // hold primitives only — safe to share.
     ...
     *
     * <p>Mirrors the retired {@code PointerGraphResolver}'s resolvePointer
     * arms: PE-style (path only) vs member-style (path + element).</p>
     */
    private static Object resolvePointerToOriginal(Object ptr, String path, Context ctx)
    {
        Object elementObj = _Any.readBySlot(ptr, SLOT_ELEMENT);
        if (elementObj instanceof String elementName && !elementName.isEmpty())
        {
            ...
            String pointerType = _Any.pureTypeOf(ptr);
