...
public final class _GenericType
{
    ...
    private static final int SLOT_RETURN_TYPE = org.finos.legend.pure.truffle.compiler.module.PureClassRegistry.globalSlot("returnType");
    ...
    public static Object type(Object gt)
    {
        ...
        return _Any.readBySlot(gt, SLOT_TYPE);
    }
    ...
    public static PureSequence typeArguments(Object gt)
    {
        Object ta = gt instanceof PureDynamicObject pdo
                ...
                : _Any.readBySlot(gt, SLOT_TYPE_ARGUMENTS);
        return ta instanceof PureSequence seq ? seq : null;
    }

    public static Object buildUserDefinedGenericType(Object type, MetadataAccess resolver)
    {
        Object gt = org.finos.legend.pure.truffle.execution.TruffleInstanceFactory.createInstance(
                "meta::pure::metamodel::type::generics::UserDefinedGenericType", resolver);
        if (type != null)
        {
            _Any.write(gt, "type", type);
        }
        ...
        if (resolver != null)
        {
            ...
            if (canonical != null)
            {
                _Any.write(gt, "classifierGenericType", canonical);
            }
        }
        return gt;
    }
    ...
    public static String print(Object gt, MetadataAccess resolver)
    {
        if (gt == null)
        {
            return "null";
        }
        boolean isGtv = _Any.isType(gt, GENERIC_TYPE_VALUE_PATH, resolver);
        if (!isGtv)
        {
            ...
            Object n = _Any.readBySlot(gt, SLOT_NAME);
        ...
        if (rawType != null && _Any.pureTypeIs(rawType, FUNCTION_TYPE_PATH))
        {
            sb.append("{");
            Object paramsObj = _Any.readBySlot(rawType, SLOT_PARAMETERS);
            if (paramsObj instanceof PureSequence params)
            {
                for (int i = 0; i < params.size(); i++)
                {
                    ...
                    if (_Any.pureTypeIs(p, VARIABLE_EXPRESSION_PATH))
                    {
                        sb.append(print(_Any.readBySlot(p, SLOT_GENERIC_TYPE), resolver));
                    }
                    else
                    {
                        sb.append("?");
            ...
            sb.append("->");
            sb.append(print(_Any.readBySlot(rawType, SLOT_RETURN_TYPE), resolver));
            sb.append("}");
        }
        else if (rawType != null)
        {
            ...
            {
                Object n = _Any.readBySlot(rawType, SLOT_NAME);
