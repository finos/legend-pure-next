...
package org.finos.legend.pure.truffle.interfaces.ide.backend;
 ...
 *       invoked through {@link PureRuntime#execute}. The runtime's resolver
 ...
 *       executed via {@link PureRuntime#execute}.</li>
 * </ol>
 *
 * <p>The Java-direct {@code ModuleRegistry} + {@code FunctionDefinition} passed in
 ...
 *
 * <h3>Lifecycle</h3>
 * One {@link PureRuntime} is built at backend construction and reused
...
public final class TruffleBackend implements PureBackend
{
    ...
    // Pinning rationale: PureRuntime enters its polyglot context on
    ...
    public TruffleBackend(Path pdbDir) throws IOException
    {
        ...
        try
        {
            ...
            {
                PdbModule coreLoader = new PdbModule(corePdb);
                PdbModule compilerLoader = new PdbModule(compilerPdb);
                ...
                PureRuntime rt = PureRuntime.builder()
                        .withRegistry(reg)
                        .withCompileImmediately(false)
                        .build();
                return new Object[]{reg, rt};
            }).get();
            this.registry = (ModuleRegistry) built[0];
            this.runtime = (PureRuntime) built[1];
        }
    ...
    @Override
    public ExecutionResult execute(MutableList<SourceModule> editableModules,
                                   org.finos.legend.pure.m3.module.ModuleRegistry javaRegistry,
    ...
    private ExecutionResult doExecute(MutableList<SourceModule> editableModules,
                                      FunctionDefinition function,
                                      ProgressSink progress,
                                      ValueSpecification... args)
    {
        ...
        try
        {
            ...
            for (SourceModule module : editableModules)
            {
                ...
                {
                    progress.post(new ProgressEvent("Parsing " + module.name(), 0, 0,
                            sourceFolder.toString()));
                    Object parsedFiles = runtime.execute(parseDirFn, sourceFolder.toAbsolutePath().toString());
                    progress.post(new ProgressEvent("Compiling " + module.name(), 0, 0, null));
                    ...
                    String memModuleName = module.name() + "-mem";
                    ...
                    if (elementsField instanceof PureSequence elements)
                    {
                        ...
                        for (Module existing : registry.modules())
                        {
                            deps.add(existing.name());
                        }
                        registry.register(new InMemoryModule(memModuleName, deps, elements, registry));
                    }
                }
            }
        ...
        if (err != null)
        {
            ...
            // produced the error.
            String pureStack = org.finos.legend.pure.truffle.execution.PureStackFormatter.format(err);
     ...
     * by a {@link InMemoryModule} that we registered on a previous
     ...
     */
    private boolean alreadyInResolver(SourceModule module)
    {
        for (String path : module.elementPaths())
        {
            Module owner = registry.moduleOfPath(path);
            return owner instanceof PdbModule;
        }
        return false;
    }
