...
package org.finos.legend.pure.truffle.runtime.pdo;

import com.oracle.truffle.api.CompilerDirectives.TruffleBoundary;
import com.oracle.truffle.api.interop.TruffleObject;
import org.finos.legend.pure.truffle.runtime.PropertyAccessor;
import org.finos.legend.pure.truffle.runtime.module.PureClassInfo;
import org.finos.legend.pure.truffle.runtime.module.PureClassRegistry;
import org.finos.legend.pure.truffle.runtime.module.TruffleMetadataAccess;
import org.finos.legend.pure.truffle.runtime.module.pdbModule.GenericFbDecoder;
 ...
 * path). On first read of a property whose slot is {@link #LAZY},
 * the value is materialised via the schema-driven
 * {@link GenericFbDecoder} and
...
public class PureDynamicObject implements PropertyAccessor, TruffleObject
{
    ...
    /** Slot sentinel: "not yet materialised from the backing FlatBuffer".
     *  Distinct from {@code null}, which means a genuinely absent/unset
     *  value. Lives here because it is intrinsic to the slots encoding. */
    public static final Object LAZY = new Object();

    /** Per-instance property storage, indexed by {@link PureClassInfo#slotIndex}.
     *  Sized to {@code classInfo.slotCount} at construction. FB-backed
     *  instances start with every slot set to {@link #LAZY}
     *  (decode-on-first-read). Pure-built instances start with {@code null}
     *  in every slot. NON-final: a PDO constructed mid-bootstrap (its
     *  classInfo still populating — the BUILDING guard hands out partial
     *  info) sizes short; {@link #ensureSlotCapacity} grows it on the first
     *  write past the stale length once the SHARED classInfo has filled in. */
    public Object[] slots;
    ...
    {
        ensureSlotCapacity(slotIdx);
        slots[slotIdx] = value;
    }

    /**
     * Grow {@link #slots} when a write targets a slot past the stale length
     * of a mid-bootstrap-constructed PDO (see the {@link #slots} javadoc).
     * Grown cells start LAZY for FB-backed instances so later reads still
     * materialise from the FlatBuffer instead of reading a false null.
     */
    private void ensureSlotCapacity(int slotIdx)
    {
        if (slotIdx < slots.length)
        {
            return;
        }
        int newLength = Math.max(classInfo.slotCount(), slotIdx + 1);
        Object[] grown = Arrays.copyOf(slots, newLength);
        if (fb != null)
        {
            Arrays.fill(grown, slots.length, newLength, LAZY);
        }
        slots = grown;
    }
    ...
    private Object decodeFromFb(String name)
    {
        ...
        // NOTE: no `fb instanceof PropertyAccessor` legacy branch here. The
        // XPDBHelper backings it served are gone, and keeping it is not just
        // dead code: with PureDynamicObject as (almost) the only remaining
        // PropertyAccessor implementor, partial evaluation devirtualizes the
        // readProperty call and inlines the phantom readSlot→decodeFromFb→
        // readProperty cycle to unbounded depth (observed: parser-mapping
        // functions bailing GraphTooBig at any cap; at a 1M cap the bailout
        // became "Too deep inlining ... recursive inlining", 331 levels).
        // Direct call — the schema-driven decoder is the ONE decode
        // implementation (the per-class Decoder registry it replaced served
        // the 74 generated helpers, all retired). Also the simplest shape
        // for partial evaluation: a static call into a @TruffleBoundary.
        return GenericFbDecoder.decode(
                name, fb, (TruffleMetadataAccess) resolver, this);
    }
