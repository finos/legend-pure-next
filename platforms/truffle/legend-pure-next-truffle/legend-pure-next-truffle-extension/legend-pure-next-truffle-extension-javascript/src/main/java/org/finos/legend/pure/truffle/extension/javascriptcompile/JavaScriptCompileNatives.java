...
/**
 * GraalJS runtime bridge for translated Pure code. Two phases:
 *
 * <ul>
 *   <li>{@link #compile} — eval the (plain JS) module source as a CommonJS
 *       module → return a {@link JavaScriptCompilationContext} wrapping
 *       {@code module.exports}. The translator emits type-free JavaScript, so
 *       there is no transpile step — the source is eval'd directly.</li>
 *   <li>{@link #invoke} — look up {@code fnName} on the captured exports,
 *       call with args, map the return value back to Pure types.</li>
 * </ul>
 *
 * <p>Holds a singleton GraalJS {@link Context} with {@code runtime-lib.js} and
 * the antlr bundle pre-loaded. Also binds a global
 ...
 */
final class JavaScriptCompileNatives
{
    private static final String RUNTIME_LIB_JS_RESOURCE = "/runtime-lib.js";
    /** Vendored antlr4 runtime + generated JS Top/M3 parsers + the 17
     *  ANTLR-bridge native impls, bundled by
     *  `just modules::translation_javascript::antlr-all`. Eval'd once at Context
     *  init so the translated parser-mappings (which calls `__parseAntlr` /
     *  `__getChild` / …) runs without leaving GraalJS. Same JS ships to the
     *  browser later — the bundle is target-neutral. */
    private static final String ANTLR_BUNDLE_JS_RESOURCE = "/antlr-bundle.js";
    /** Translated parser-mappings interpreter + `__pureParseTop`, bundled by
     *  `just modules::translation_javascript::parser-mappings-bundle`. OPTIONAL:
     *  producing it needs pure-truffle and the javascript/translation PDBs, which
     *  do not exist when this jar is packaged (CI builds the jar upstream of them).
     *  Absent, everything works except Pure's `parse` — see {@link #context()}. */
    private static final String PARSER_MAPPINGS_BUNDLE_JS_RESOURCE = "/parser-mappings-bundle.js";

    private static final Object LOCK = new Object();
    private static Context jsContext;
    private static String runtimeLib;
    private static String antlrBundle;
    ...
    /** Per-execute registry of DYNAMIC graph roots — compileSource results.
     *  Each result registers under a unique `__dyn::<n>` root so the generic
     *  sub-address walker serves all downstream reads/instanceOf/evaluate;
     *  cleared with INJECTED_GRAPH, so dynamic values are valid only within
     *  the execute(...) call that compiled them (the compileSource contract). */
    static final ThreadLocal<java.util.Map<String, Object>> DYNAMIC_GRAPHS =
            ThreadLocal.withInitial(java.util.HashMap::new);
    private static final ThreadLocal<int[]> DYNAMIC_COUNTER = ThreadLocal.withInitial(() -> new int[1]);

    /** Register a compileSource result as an addressable dynamic graph root. */
    static String registerDynamicGraph(Object root)
    {
        String address = "__dyn::" + (DYNAMIC_COUNTER.get()[0]++);
        DYNAMIC_GRAPHS.get().put(address, root);
        return address;
    }
    ...
    private static final ThreadLocal<TruffleMetadataAccess> ACTIVE_RESOLVER =
            new ThreadLocal<>();

    /** PureContext snapshot for the active {@code invoke} call. The
     *  {@code __metadataInvoke} bridge needs a live {@code PureContext} to
     *  pass into {@link org.finos.legend.pure.truffle.interpreter.ast.natives.lang.EvalNode#dispatch}
    ...
    /** Every JS module string handed to {@link #compile} is appended here so
    ...
    private JavaScriptCompileNatives() {}
    ...
    static JavaScriptCompilationContext compile(String source)
    {
        ...
        synchronized (LOCK)
        {
            ...
            // large embedded JS strings. (The antlr-bundle.js is evaled ONCE at
            // Context init — see `context()` — not per compile call.)
            //
            // The translator emits plain JS (no type syntax), so there is
            // nothing to transpile — we eval directly. The one `export function
            // execute` is the module entry point; strip the `export` keyword
            // (the only export the translator emits) and surface it on
            // module.exports for the CommonJS-style `new Function` eval below.
            String body = runtimeLib()
                    + source.replaceAll("(?m)^export ", "")
                    + "\n;if (typeof execute !== 'undefined') { module.exports.execute = execute; }\n";
            return new JavaScriptCompilationContext(evalToExports(ctx, body));
        }
    }

    static Object invoke(JavaScriptCompilationContext compCtx, String fnName, Object[] args, Object graph)
    {
        synchronized (LOCK)
        {
            ...
            TruffleMetadataAccess resolver = currentResolver();
            ...
            finally
            {
                ...
                DYNAMIC_GRAPHS.remove();
                DYNAMIC_COUNTER.remove();
    ...
    private static String runtimeLib()
    {
        if (runtimeLib == null)
        {
            try (InputStream in = JavaScriptCompileNatives.class.getResourceAsStream(RUNTIME_LIB_JS_RESOURCE))
            {
                if (in == null)
                {
                    throw new RuntimeException(
                            "runtime-lib.js not found on classpath at '" + RUNTIME_LIB_JS_RESOURCE +
                            "'. The pom copies it from pure/modules/translation/javascript/js/runtime-lib.js " +
            ...
            {
                throw new RuntimeException("Failed to read bundled runtime-lib.js", e);
            }
        }
        return runtimeLib;
    }
    ...
    private static String antlrBundle()
    {
        if (antlrBundle == null)
        {
            try (InputStream in = JavaScriptCompileNatives.class.getResourceAsStream(ANTLR_BUNDLE_JS_RESOURCE))
            {
                if (in == null)
                {
                    throw new RuntimeException(
                            "antlr-bundle.js not found on classpath at '" + ANTLR_BUNDLE_JS_RESOURCE +
                            "'. Build pre-req: run `just modules::translation_javascript::antlr-all` " +
                            "to generate pure/modules/translation/javascript/js/build/antlr-bundle.js " +
                            "before building this extension.");
                }
                antlrBundle = new String(in.readAllBytes(), StandardCharsets.UTF_8) + "\n";
            }
            catch (IOException e)
            {
                throw new RuntimeException("Failed to read bundled antlr-bundle.js", e);
            }
        }
        return antlrBundle;
    }

    /** The parser-mappings bundle, or {@code null} when it was not baked into
     *  the jar. Unlike {@link #antlrBundle()} a miss is NOT fatal: only Pure's
     *  `parse` needs this half, and the jar is deliberately packagable without
     *  it so the build does not depend on PDBs that come later. */
    private static String parserMappingsBundle()
    {
        try (InputStream in = JavaScriptCompileNatives.class.getResourceAsStream(PARSER_MAPPINGS_BUNDLE_JS_RESOURCE))
        {
            return in == null ? null : new String(in.readAllBytes(), StandardCharsets.UTF_8) + "\n";
        }
        catch (IOException e)
        {
            throw new RuntimeException("Failed to read bundled parser-mappings-bundle.js", e);
        }
    }
    ...
    private static Context context()
    {
        if (jsContext == null)
        {
            Context c = Context.newBuilder("js")
                    ...
                    .allowAllAccess(true)
                    .build();
            // Runtime-lib (plain JS) — eval'd at Context init so its
            // `function __ctorScope(...)` etc. land on globalThis. The
            // antlr-bundle's parser-mappings code references these helpers
            // by bare name; they resolve through the global scope chain
            // because parser-mappings was bundled SEPARATELY from runtime-lib
            // (zero exports → tree-shaken if imported). Per-compile user
            // source still gets runtime-lib prepended (see compile()) — the
            // global functions and the prepended copies don't conflict
            // (function declarations re-bind without error).
            c.eval(Source.newBuilder("js", runtimeLib(), "runtime-lib.js").buildLiteral());
            // ANTLR bundle — evaled ONCE at Context init so its 944KB IIFE
            // doesn't get re-transpiled per compile call. Sets globalThis
            // members __parseAntlr / __getText / __grammarRuleName / … that
            // the translated parser-mappings code calls. See
            // pure/modules/translation/javascript/js/antlr-entry.js.
            c.eval(antlrBundleSource());
            // Parser-mappings half — the translated meta::pure::parser::mappings
            // interpreter plus `__pureParseTop`. Split from the antlr bundle
            // because it needs PDBs that do not exist when this jar is built;
            // see parser-mappings-entry.js. When it was not bundled, install a
            // stub so the only casualty is Pure's `parse`, and it fails with a
            // build instruction rather than "__pureParseTop is not a function".
            String mappings = parserMappingsBundle();
            if (mappings != null)
            {
                c.eval(Source.newBuilder("js", mappings, "parser-mappings-bundle.js").buildLiteral());
            }
            else
            {
                c.eval(Source.newBuilder("js",
                        "globalThis.__pureParseTop = function () {"
                        + " throw new Error('parser-mappings-bundle.js was not bundled into this"
                        + " extension jar, so Pure\\'s parse() is unavailable. Build pre-req: run"
                        + " `just modules::translation_javascript::parser-mappings-bundle`, then"
                        + " rebuild the extension.'); };",
                        "parser-mappings-stub.js").buildLiteral());
            }
            ...
            // compileSource host implementation (runtime-lib's __compileSource
            // delegates here): routes to the interpreter-side native via
            // TruffleEvalBridge — same contract, same messages, no global state.
            c.getBindings("js").putMember("__hostCompileSource",
                    (java.util.function.BiFunction<Object, Object, Object>)
                            (Object file, Object deps) -> TruffleEvalBridge.compileSource(
                                    file, deps, activeResolver(), ACTIVE_CONTEXT.get()));
            // ─────────────────────────────────────────────────────────────
            jsContext = c;
        }
        return jsContext;
    ...
    private static TruffleMetadataAccess currentResolver()
    {
    ...
    private static TruffleMetadataAccess activeResolver()
    {
        TruffleMetadataAccess r = ACTIVE_RESOLVER.get();
        if (r == null)
        {
                                       ...
                                       TruffleMetadataAccess resolver)
    {
        ...
        Object value = org.finos.legend.pure.truffle.runtime.helper._Any.read(node, prop);
    ...
    private static String classifierPathOf(Object element,
                                           TruffleMetadataAccess resolver)
    {
        Object cgt = unwrapSingle(org.finos.legend.pure.truffle.runtime.helper._Any.read(element, "classifierGenericType"));
        if (cgt == null) return null;
        Object type = unwrapSingle(org.finos.legend.pure.truffle.runtime.helper._Any.read(cgt, "type"));
                                         ...
                                         TruffleMetadataAccess resolver)
    {
        ...
        else if (pePath.startsWith("__dyn::"))
        {
            pe = DYNAMIC_GRAPHS.get().get(pePath);
            if (pe == null)
            {
                throw new RuntimeException("__metadataRead: dynamic graph '" + pePath
                        + "' is not registered (compileSource results are valid only within the execute(...) call that compiled them)");
            }
        }
        else
        {
     ...
     *       org.finos.legend.pure.truffle.runtime.helper._Any#read} on
     *       the current node.</li>
     * </ul>
     */
    private static Object walkSubPath(Object root, String subPath)
    {
        ...
        for (String seg : subPath.split("/"))
        {
            ...
            current = org.finos.legend.pure.truffle.runtime.helper._Any.read(current, seg);
            if (current == null)
            {
    ...
    private static void collectLambdasInOrder(Object node, java.util.List<Object> out)
    {
        ...
        Object exprs = org.finos.legend.pure.truffle.runtime.helper._Any.read(node, "expressionSequence");
        ...
        Object val = org.finos.legend.pure.truffle.runtime.helper._Any.read(node, "value");
        if (val != null)
        {
            if (org.finos.legend.pure.truffle.runtime.helper._Any.isType(val,
        ...
        Object params = org.finos.legend.pure.truffle.runtime.helper._Any.read(node, "parametersValues");
        ...
        // Collection.values
        Object vals = org.finos.legend.pure.truffle.runtime.helper._Any.read(node, "values");
                                        ...
                                        TruffleMetadataAccess resolver)
    {
        ...
        if (value instanceof org.finos.legend.pure.truffle.runtime.pdo.PureDynamicObject pdo)
        {
    ...
    /** ANTLR bundle source: vendored antlr4 + generated JS parsers + 17
     *  native impls, wrapped in an IIFE that publishes the natives on
     *  globalThis. See pure/modules/translation/javascript/js/antlr-entry.js. */
    private static Source antlrBundleSource()
    {
        return Source.newBuilder("js", antlrBundle(), "antlr-bundle.js").buildLiteral();
    }

    // Wraps the JS in an IIFE that provides a CommonJS-style
                                      ...
                                      TruffleMetadataAccess resolver)
    {
        ...
        if (v.hasMembers())
        {
            ...
            if (v.hasMember("__mapEntries"))
            {
                ...
                org.finos.legend.pure.truffle.types.MapImpl map =
                        new org.finos.legend.pure.truffle.types.MapImpl();
            ...
            // Variant marker (`__variantJson`): the runtime-lib wraps a JSON
            // tree (null | boolean | bigint | number | string | Array | Map).
            // Lift to the shared PureVariant so toString/equality behave
            // identically to the native Truffle Variant nodes.
            if (v.hasMember("__variantJson"))
            {
                return new org.finos.legend.pure.execution.PureVariant(
                        jsJsonTreeToJsonValue(v.getMember("__variantJson")));
            }
        ...
        throw new RuntimeException("Unsupported JavaScript return type: " + v);
    }

    /**
     * Convert a runtime-lib Variant JSON tree (crossed via interop) to the
     * shared {@link org.finos.legend.pure.execution.JsonValue}. Trees are:
     * null | boolean | bigint (JSON integer) | number (JSON decimal) |
     * string | Array | JS Map (JSON object).
     */
    @com.oracle.truffle.api.CompilerDirectives.TruffleBoundary
    private static org.finos.legend.pure.execution.JsonValue jsJsonTreeToJsonValue(Value v)
    {
        if (v == null || v.isNull())
        {
            return org.finos.legend.pure.execution.JsonValue.JsonNull.INSTANCE;
        }
        if (v.isBoolean())
        {
            return org.finos.legend.pure.execution.JsonValue.JsonBoolean.of(v.asBoolean());
        }
        if (v.isString())
        {
            return new org.finos.legend.pure.execution.JsonValue.JsonString(v.asString());
        }
        if (v.isNumber())
        {
            // Integers are bigints in the tree; decimals are JS numbers. A
            // whole-valued decimal also fits a long — classify by the double
            // round-trip only when the value is NOT integral.
            if (v.fitsInLong() && (!v.fitsInDouble() || v.asDouble() == Math.rint(v.asDouble())))
            {
                // bigint or whole number: bigints must render without ".0",
                // so probe the JS-side type via Value metadata — a JS number
                // has meta simple name "number"; keep whole JS numbers decimal
                Value meta = v.getMetaObject();
                boolean isJsNumber = meta != null && "number".equals(meta.getMetaSimpleName());
                return isJsNumber
                        ? org.finos.legend.pure.execution.JsonValue.JsonNumber.ofDecimal(v.asDouble())
                        : org.finos.legend.pure.execution.JsonValue.JsonNumber.ofIntegral(v.asLong());
            }
            if (v.fitsInBigInteger() && !v.fitsInDouble())
            {
                return org.finos.legend.pure.execution.JsonValue.JsonNumber.ofIntegral(v.asBigInteger().longValueExact());
            }
            return org.finos.legend.pure.execution.JsonValue.JsonNumber.ofDecimal(v.asDouble());
        }
        if (v.hasArrayElements())
        {
            long n = v.getArraySize();
            java.util.List<org.finos.legend.pure.execution.JsonValue> elements = new java.util.ArrayList<>((int) n);
            for (long i = 0; i < n; i++)
            {
                elements.add(jsJsonTreeToJsonValue(v.getArrayElement(i)));
            }
            return new org.finos.legend.pure.execution.JsonValue.JsonArray(elements);
        }
        if (v.hasHashEntries())
        {
            java.util.LinkedHashMap<String, org.finos.legend.pure.execution.JsonValue> fields = new java.util.LinkedHashMap<>();
            Value iterator = v.getHashEntriesIterator();
            while (iterator.hasIteratorNextElement())
            {
                Value entry = iterator.getIteratorNextElement();
                fields.put(entry.getArrayElement(0).asString(), jsJsonTreeToJsonValue(entry.getArrayElement(1)));
            }
            return new org.finos.legend.pure.execution.JsonValue.JsonObject(fields);
        }
        throw new RuntimeException("Unsupported Variant JSON tree node: " + v);
    }
    ...
    // object. Trying to _Any.write each JS key onto a Map PDO fails;
                                    ...
                                    TruffleMetadataAccess resolver)
    {
        ...
        if (cls instanceof org.finos.legend.pure.truffle.runtime.pdo.PureDynamicObject pdoCls
                && "meta::pure::metamodel::type::Enumeration".equals(pdoCls.purePath()))
        {
            ...
            {
                org.finos.legend.pure.truffle.runtime.pdo.PureDynamicObject enumPdo =
                        new org.finos.legend.pure.truffle.runtime.pdo.PureDynamicObject(
                                org.finos.legend.pure.truffle.runtime.module.PureClassRegistry.classInfoFor(
                                        "meta::pure::metamodel::type::Enum", resolver),
                                null, resolver, null);
                enumPdo.writeProperty("name", enumValueName);
                org.finos.legend.pure.truffle.runtime.module.EnumValueSingletons.register(enumPdo, classPath);
                return enumPdo;
            }
        }
        ...
        org.finos.legend.pure.truffle.runtime.helper._Any.write(instance, "classifierGenericType", cgt);
        for (String key : v.getMemberKeys())
        {
            ...
            org.finos.legend.pure.truffle.runtime.helper._Any.write(instance, key,
