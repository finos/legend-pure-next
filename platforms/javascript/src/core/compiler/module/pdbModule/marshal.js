...
// runtime expects, and build the metadata services it calls (`metadataHost`).
//
...
function rehydrateAtomicEnum(raw, genericType) {
    ...
    if (typeof typePath !== "string"
            || !globalThis.__pureHost?.metadataInstanceOf
            || !globalThis.__pureHost.metadataInstanceOf(typePath, "meta::pure::metamodel::type::Enumeration")) {
        return raw;
    }
...
export function marshal(sx, v, stack = []) {
    ...
    if (isPointerRef(v)) {
        ...
        if (kind === "Property" || kind === "QualifiedProperty") {
            ...
            const resolved = () => {
                const props = globalThis.__pureHost.metadataRead(ownerPath, "properties");
        ...
        if (globalThis.__pureHost?.metadataPathToElement
                && globalThis.__pureHost.metadataPathToElement(p, "::") === null
...
function resolveLambdaAst(store, address) {
    ...
    if (typeof collect !== "function" || !Number.isInteger(idx) || !store.hasElement(basePath)) return undefined;
    const exprSeq = globalThis.__pureHost.metadataRead(basePath, "expressionSequence");
...
function synthRoot(store, prop) {
    ...
    if (prop === "children") {
        ...
        for (const m of store.modules ?? []) {
            for (const p of m.elementPaths()) if (!p.includes("::")) seen.add(p);
        }
...
function readProp(store, address, prop) {
        ...
        if (address.includes(LAMBDA_SEP)) {
            ...
            if (lam !== undefined && lam !== null) {
                ...
                if (typeof lam === "function" && typeof lam.__purePath === "string" && lam.__purePath !== address) {
                    return globalThis.__pureHost.metadataRead(lam.__purePath, prop);
                }
                return lam[prop];
            }
        }
    ...
    if (node.live) {
        ...
        if (typeof obj === "function" && typeof obj.__purePath === "string" && obj.__purePath !== address) {
            return globalThis.__pureHost.metadataRead(obj.__purePath, prop);
        }
        return obj[prop];
    }
...
// The metadata services the PureRuntime puts on its host object (runtime-lib's `__metadataRead`,
// `__metadataSubtypeOf`, … forward to them).
export function metadataHost(store, opts = {}) {
    const log = opts.log ?? [];
    ...
    // which has no PDB).
    const metadataSubtypeOf = (sub, sup) => store.subtypeOf(sub, sup);

    const metadataInstanceOf = (valuePath, typePath) => store.instanceOf(valuePath, typePath);
    ...
    const metadataRead = (address, prop) => {
    ...
    const metadataInvoke = (path, args) => { rec({ fn: "invoke", path }); return store.invoke(path, args); };
    ...
    const findAllTypes = () => store.allTypes().map((p) => globalThis.__pureResolve(p));
    const findFunctionsByNameAndArity = (name, arity) =>
        store.functionsByNameAndArity(name, arity).map((p) => globalThis.__pureResolve(p));

    return { metadataSubtypeOf, metadataInstanceOf, metadataPathToElement, metadataRead, metadataInvoke, findAllTypes, findFunctionsByNameAndArity };
}
