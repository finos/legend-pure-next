...
export { PdbModule } from "./pdbModule/PdbModule.js";
export { InMemoryModule } from "./inMemoryModule/InMemoryModule.js";

// ModuleRegistry's state: resolution across an ordered module list (first
// match wins), the caches, and the graph/function-index queries. `schemaSrc` is
// the m3.fbs text (parsed lazily via the translated parser) or an already-parsed
// schema index.
function createRegistryState(schemaSrc) {
...
/**
 * The canonical module registry: ordered first-match resolution over the
 * registered modules (MetadataAccess), plus the graph and function-index
 * services the metadata globals consume (pdb/marshal.js). JavaScript extra: the
 * constructor takes the m3.fbs schema (text, parsed lazily, or a parsed index).
 */
export class ModuleRegistry {
    ...
    unregister(name) {
        ...
        if (module) this.#state.unregister(module);
        return module;
    }
    ...
    module(name) { return this.modules.find((m) => m.name === name) ?? null; }

    // --- MetadataAccess -------------------------------------------------------
    hasElement(path) { return this.#state.has(path); }
    ...
    get elementCount() { return this.#state.elementCount; }
    addInvalidationListener(listener) { this.#state.addInvalidationListener(listener); }
    resolve(path) { return this.#state.resolve(path); }
    resolveAll(path) { return this.#state.resolveAll(path); }
    subtypeOf(sub, sup) { return this.#state.subtypeOf(sub, sup); }
    instanceOf(valuePath, typePath) { return this.#state.instanceOf(valuePath, typePath); }
    classifierPath(path) { return this.#state.classifierPath(path); }
    directSupers(path) { return this.#state.directSupers(path); }
    allTypes() { return this.#state.allTypes(); }
    functionsByNameAndArity(name, arity) { return this.#state.functionsByNameAndArity(name, arity); }
    invoke(path, args) { return this.#state.invoke(path, args); }
}
