...
// is already here (module/pdbModule/marshal.js installs __pureResolve and the
...
import { EVALUATE, JS_COMPILE_MODULE, JS_DRAIN_COMPILED_SOURCES, JS_EXECUTE } from "./native-signatures.js";

/**
 * Host natives of the javascript language module (meta::external::language::javascript):
 * compileModule, execute and drainCompiledSources, plus evaluate of a function
 * definition built at run time.
 */
export const javascriptLanguageExtension = {
    registerAll(natives) {
        natives.register(JS_COMPILE_MODULE, (runtime) => hostJsNatives(runtime).compileModule);
        natives.register(JS_EXECUTE, (runtime) => hostJsNatives(runtime).execute);
        natives.register(JS_DRAIN_COMPILED_SOURCES, (runtime) => hostJsNatives(runtime).drainCompiledSources);
        natives.register(EVALUATE, (runtime) => hostJsNatives(runtime).evaluateFunctionDefinition);
    },
};

// One set of closures per runtime: compileModule and drainCompiledSources share
// the capture buffer.
const perRuntime = new WeakMap();
function hostJsNatives(runtime) {
    let natives = perRuntime.get(runtime);
    if (!natives) perRuntime.set(runtime, natives = createHostJsNatives(runtime.evalJs, runtime.runtimeModule));
    return natives;
}

/**
 * Provide hostJsCompile / hostJsExecute / hostJsDrainCompiledSources on the runtime's host object.
 *
...
function createHostJsNatives(evalJs, memModule = null) {
    ...
    const evaluateFunctionDefinition = (fn, args) => {
        if (args.length !== 0) {
    ...
    const execute = (ctx, fnName, args, _pureReturnType, _pureMultiplicity, graph) => {
    ...
    return { compileModule, execute, drainCompiledSources, evaluateFunctionDefinition };
}
