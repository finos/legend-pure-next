...
public final class ScopedMetadataAccess implements MetadataAccess
{
    ...
    private final Map<meta.pure.metamodel.PackageableElement, String> elementPathCache = new IdentityHashMap<>();

    // Positive-only resolution cache. JFR found ~9.5% of self-compile CPU
    // in CompressedArchiveReader.hasElement, traced to the duplicated
    // hasElement+getElement work below. We can only cache *positive*
    // results because a path that is missing now may be found later —
    // the compiler-pure runtime can extend the resolved set during a
    // single compile (e.g. lazily-resolved IR cross-references).
    ...
    @Override
    public Map<meta.pure.metamodel.PackageableElement, String> elementPathCache()
    {
