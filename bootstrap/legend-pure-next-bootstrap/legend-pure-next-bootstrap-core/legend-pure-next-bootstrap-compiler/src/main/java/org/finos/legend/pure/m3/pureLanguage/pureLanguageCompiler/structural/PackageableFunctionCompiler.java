...
public final class PackageableFunctionCompiler
{
            ...
            CompilationContext context)
    {
        ...
        // reuses the same TypeParameter/MultiplicityParameter objects for references
        ...
        result._parameters(grammar._parameters()
                        .collect(p -> VariableCompiler.compileParameter(p, imports, model, context))
                        .select(Objects::nonNull));

        _Function.validateFunctionParameters(result._parameters(), context, SourceInformationCompiler.compile(grammar._p_sourceInformation(), context.getSourceId(), model));
