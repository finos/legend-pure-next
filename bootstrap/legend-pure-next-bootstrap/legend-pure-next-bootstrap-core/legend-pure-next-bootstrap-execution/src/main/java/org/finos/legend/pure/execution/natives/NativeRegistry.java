...
public class NativeRegistry
{
    ...
    /** The runtime's compilation, reached lazily by {@code compileSource}; null outside a runtime. */
    private final java.util.function.Supplier<org.finos.legend.pure.m3.compilation.Compilation> compilation;
    /**
     ...
     * {@link NativeRegistry} instance.
     */
    private final Map<String, GrammarExtension> grammars;

    public NativeRegistry(MetadataAccess resolver)
    {
        this(resolver, null, List.of(), List.of(), null);
    }

    private NativeRegistry(MetadataAccess resolver,
                             Iterable<? extends NativesExtension> extensions,
                             List<? extends ParserExtension> parserExtensions,
                             List<? extends GrammarExtension> grammarExtensions,
                             java.util.function.Supplier<org.finos.legend.pure.m3.compilation.Compilation> compilation)
    {
        ...
        // NativeRegistry — Builder.build() OR the back-compat
        // `new NativeRegistry(resolver)` — gets them automatically. Explicit
        ...
        if (extensions != null)
        {
            extensions.forEach(ext -> ext.registerAll(this));
        }
    }
    ...
    public static class Builder
    {
        ...
        private java.util.function.Supplier<org.finos.legend.pure.m3.compilation.Compilation> compilation;
        ...
        /** The runtime's compilation strategy, used by {@code compileSource}. */
        public Builder withCompilation(java.util.function.Supplier<org.finos.legend.pure.m3.compilation.Compilation> compilation)
        {
            this.compilation = compilation;
            return this;
        }

        public NativeRegistry build()
        {
            return new NativeRegistry(resolver, nativeExtensions, parserExtensions, grammarExtensions, compilation);
        }
    }
    ...
    /** Register an eager native under its mangled signature. */
    public NativeRegistry register(String signature, NativeImpl impl)
    {
        natives.put(signature, impl);
        return this;
    }

    /**
     * Register a lazy native — it receives the unevaluated {@link FunctionExpression}
     * and controls argument evaluation (short-circuit operators, special forms).
     * Bootstrap extra.
     */
    public NativeRegistry registerLazy(String signature, LazyNativeImpl impl)
    {
        lazyNatives.put(signature, impl);
        return this;
    }
    ...
    /**
     * Resolved binding for a native call site — exactly one of {@code impl}
     * (eager) or {@code lazyImpl} (lazy) is non-null. {@link #binding} caches
     * these per signature so the hot dispatch path is one
     * {@code ConcurrentHashMap.get} instead of one {@code containsKey} +
     * one {@code get}.
     */
    private static final class Binding
    {
        final NativeImpl impl;
        final LazyNativeImpl lazyImpl;

        Binding(NativeImpl impl, LazyNativeImpl lazyImpl)
        {
            this.impl = impl;
            this.lazyImpl = lazyImpl;
        }
    }

    private final java.util.concurrent.ConcurrentHashMap<String, Binding> bindingCache =
            new java.util.concurrent.ConcurrentHashMap<>();

    private Binding binding(String signature)
    {
        Binding b = bindingCache.get(signature);
        if (b != null)
        {
            return b;
        }
        LazyNativeImpl lazy = lazyNatives.get(signature);
        if (lazy != null)
        {
            return bindingCache.computeIfAbsent(signature, s -> new Binding(null, lazy));
        }
        NativeImpl eager = natives.get(signature);
        if (eager != null)
        {
            return bindingCache.computeIfAbsent(signature, s -> new Binding(eager, null));
        }
        return null;
    }

    /**
     * Resolve a binding for the call site. The evaluator looks this up
     * once and either passes the unevaluated {@link FunctionExpression}
     * to {@link #invokeLazy} or pre-evaluates args and calls
     * {@link #invokeEager} — one cache lookup per native call.
     */
    public Object resolve(String signature)
    {
        Binding b = binding(signature);
        if (b == null)
        {
    ...
    public boolean isLazyResolved(Object resolved)
    {
        return ((Binding) resolved).lazyImpl != null;
    }

    public ValueSpecification invokeLazy(Object resolved, FunctionExpression fe, ValueSpecificationEvaluator evaluator)
    {
        Binding b = (Binding) resolved;
        try
        {
            return b.lazyImpl.apply(fe, evaluator);
        }
    ...
    public ValueSpecification invokeEager(Object resolved, List<ValueSpecification> args,
                                          ...
                                          meta.pure.metamodel.multiplicity.Multiplicity multiplicity)
    {
        ...
        try
        {
            return b.impl.apply(args, evaluator, genericType, multiplicity);
        }
        catch (PureAssertionError e)
        {
            if (!e.getMessage().contains("\nPure stack trace:"))
    ...
    public static String pureToString(Object obj)
    {
        ...
        {
            return "[" + list.stream().map(NativeRegistry::pureToString).reduce((a, b) -> a + ", " + b).orElse("") + "]";
        }
