...
public class ValueSpecificationEvaluator
{
    ...
    // linear scan over the local-frame names. With this cache, monomorphic
    // call sites (the common case: a function-local variable referenced
    // many times) become a single array index + equality check instead of
    // a full scan.
    //
    // Identity-keyed: the same {@link VariableExpression} node is shared
    // across all calls of its enclosing function, so the cached slot stays
    // valid as long as the frame's layout is stable (same FunctionDefinition
    // body, same set of params/lets evaluated). On a cache miss (different
    // frame shape) we fall through to {@link Scope#get} and update the
    // cache.
    ...
    /**
     * Push a new variable scope onto the stack with the current scope as
     * its parent. New bindings created via the returned scope shadow but
     * do not mutate the parent. Must be paired with {@link #popScope()}.
     */
    public void pushScope()
    {
        varStack.push(new Scope(varStack.peek()));
    }
    ...
    public ValueSpecification evaluate(ValueSpecification vs)
    {
        return switch (vs)
        {
            ...
            case VariableExpression ve ->
            {
                ...
                Integer cachedSlot = localSlotCache.get(ve);
                if (cachedSlot != null)
                {
                    int slot = cachedSlot;
                    if (slot < cur.localSize())
                    {
                        String n = cur.localNameAt(slot);
                        if (n == name || n.equals(name))
                        {
                            yield cur.localValueAt(slot);
                        }
                    }
                }
                // Fall back to a full chain walk
                ValueSpecification rv = cur.get(name);
                if (rv == null)
                {
                    throw new RuntimeException("Unknown variable: " + name);
                }
                // Refresh the inline cache when the value lives in the current
                // frame (the common case). Bindings in parent frames stay
                // unmemoized — they're rarer and would invalidate too easily.
                int idx = cur.localIndexOf(name);
                if (idx >= 0)
                {
                    localSlotCache.put(ve, idx);
                }
                yield rv;
            }
            case Collection col ->
    ...
    private AtomicValue captureLambdaClosureIfNeeded(AtomicValue av)
    {
        ...
        // Closure stores them in a flat (parent-less) Scope so the closure body
        // doesn't accidentally pick up unrelated frames from the original chain.
        Scope currentScope = varStack.peek();
        Scope captured = new Scope(null, openVars.size());
    ...
    private ValueSpecification evaluateFunctionExpression(FunctionExpression fe)
    {
        ...
        try
        {
            ...
            return switch (func)
            {
                ...
                {
                    Object resolved = natives.resolve(nf._name());
                    yield natives.isLazyResolved(resolved)
                            ? natives.invokeLazy(resolved, fe, this)
                            : natives.invokeEager(resolved, evaluateArgs(fe), this,
    ...
    public ValueSpecification evaluateFunctionDefinition(FunctionDefinition fd, List<ValueSpecification> args)
    {
        ...
        Scope baseScope = (fd instanceof Closure closure)
                ? closure.capturedScope()
                : varStack.peek();
        Scope childVars = new Scope(baseScope);
