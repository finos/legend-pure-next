...
import org.finos.legend.pure.runtime.PureRuntime;
...
public class PureLSPServer implements LanguageServer, TextDocumentService, WorkspaceService
{
    ...
    private final MutableList<SourceModule> editableModules;
    private final PureBackend backend;
    private String currentSource = "";
    private String currentUri = "";
    private final ModuleRegistry registry;
    private final PureRuntime runtime;
    /** The registry after the last successful compile (null until then). */
    private ModuleRegistry lastRegistry;
    ...
    public PureLSPServer(MutableList<PdbModule> pdbModules, MutableList<SourceModule> editableModules, PureBackend backend)
    {
        ...
        // One runtime over the editable modules and every PDB: the Java compiler compiles the
        // editable source modules in place (one pass) on every runtime.compile().
        this.registry = new ModuleRegistry(Lists.mutable.<Module>withAll(editableModules).withAll(pdbModules));
        this.runtime = PureRuntime.builder()
    ...
    public void initialized(InitializedParams params)
    {
        ...
        {
            SourceModule welcomeModule = findModuleForSource("welcome.pure");
     ...
     * ({@code lastRegistry}), then resolves that node's reference to a
    ...
    private org.eclipse.lsp4j.jsonrpc.messages.Either<List<? extends Location>, List<? extends LocationLink>> resolveDefinition(DefinitionParams params)
    {
        ...
        if (exprAtCursor == null)
        {
            ...
            System.err.println("[LSP/def]   lastRegistry modules:");
            for (Module mod : lastRegistry.modules())
            {
                System.err.println("[LSP/def]     " + mod.getClass().getSimpleName() + " " + mod.name() + " elementCount=" + mod.elementPaths().size());
            }
    ...
    /**
     * Find the smallest expression (or PackageableElement) in {@link #lastRegistry}
    ...
    private Object findExpressionAt(String sourceId, int line, int col)
    {
        ...
        for (Module mod : lastRegistry.modules())
        {
            ...
            if (mod instanceof SourceModule lm)
            {
     ...
     *  falling back to {@link SourceModule#getSourceIdForElement} when the
    ...
    private TargetLocation siToTarget(meta.pure.metamodel.SourceInformation si, String elementPath)
    {
        ...
        {
            for (SourceModule mod : editableModules)
            {
    ...
    {
        if (obj == null || lastRegistry == null) return obj;
    ...
    /** Walk top-level elements in lastRegistry looking for the enclosing function
    ...
    {
        if (lastRegistry == null || ve._name() == null) return null;
    ...
    private CompilationResult compileCurrentSource() throws Exception
    {
        ...
        // The PDBs are precompiled, so only the editable source modules compile per keystroke.
    ...
    private void compileAndPublishDiagnostics()
    {
        ...
        // when compile fails, SourceModule.state holds whatever parsed/compiled
    ...
    private void executeGoFunction()
    {
        ...
        try
        {
            ...
            // Make this successful compile available to other LSP features
            ...
            Module welcomeMod = registry.module("welcome");
            ...
            PureBackend.ExecutionResult execResult = backend.execute(
                    editableModules, registry, result, goFunc, executeProgressSink());
            if (execResult.ok())
            {
    ...
    private List<String> handleGetPCTAdapters()
    {
        ...
        if (lastRegistry != null)
        {
            for (Module m : lastRegistry.modules())
            {
    ...
    private Object handleRunTests(String mode, String adapterPath, List<String> testPaths)
    {
        ...
        if (lastRegistry == null)
        {
            // Compile failed (either threw, or produced errors so lastRegistry
            ...
            for (String testPath : testPaths)
            {
                ...
                result.put("error", "Cannot run: source did not compile cleanly (lastRegistry is null). Fix compile errors and re-run.");
            ...
            if (testElem instanceof FunctionDefinition fd)
            {
                ...
                try
                {
                    PureBackend.ExecutionResult execResult = "pct".equals(mode) && adapterArg != null
                            ? backend.execute(editableModules, lastRegistry, null, fd, PureBackend.ProgressSink.NOOP, adapterArg)
                            : backend.execute(editableModules, lastRegistry, null, fd, PureBackend.ProgressSink.NOOP);
                    if (execResult.ok())
                    {
    ...
    private List<Map<String, Object>> handleSearch(String query, boolean caseSensitive)
    {
        ...
        for (SourceModule module : editableModules)
        {
    ...
    private void sendPackageTree()
    {
        ...
        // on lastRegistry because SourceModule populates its own state during
        ...
        // The PDB loop below will skip any path the SourceModule already owns,
        ...
        for (PdbModule pdb : pdbModules)
        {
            ...
            if (indexBytes == null)
            {
                throw new IllegalStateException(
                        "PDB '" + pdb.name() + "' has no elementIndex section — "
    ...
    {
        // Each editable SourceModule is a top-level folder named by the
        ...
        for (SourceModule module : editableModules)
        {
            ...
            rootChildren.put(module.name(), moduleNode);
    ...
    private void handleJumpToElement(String elementPath)
    {
        ...
        for (SourceModule module : editableModules)
        {
            ...
            if (sourceId != null)
            {
                System.out.println("[LSP] Found in module " + module.name() + " with sourceId " + sourceId);
                String content = module.getSourceText(sourceId);
                if (content != null)
                {
                    try 
     ...
     */
    private SourceModule findModuleForSource(String sourceId)
    {
        for (SourceModule module : editableModules)
        {
