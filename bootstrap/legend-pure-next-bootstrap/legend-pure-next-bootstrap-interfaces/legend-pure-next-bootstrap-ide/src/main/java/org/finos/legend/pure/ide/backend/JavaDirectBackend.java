...
/**
 * The original Pure-on-Java backend. Wraps {@link PureRuntime}. Same code
...
public final class JavaDirectBackend implements PureBackend
{
                                   ...
                                   ValueSpecification... args)
    {
        ...
        Module scope = registry.module("welcome");
        if (scope == null && !registry.modules().isEmpty())
        {
            scope = registry.modules().get(0);
        }
        PureRuntime execution = PureRuntime.builder()
                .withRegistry(scope != null
                        ? new ScopedMetadataAccess(scope, registry)
                        : registry.modules().get(0))
