...
public class PureCLI
{
    ...
    private static void inspectPdb(String[] args) throws Exception
    {
        ...
        for (String pdbPath : pdbPaths)
        {
            ...
            modules.add(module);
            loadedNames.add(module.name());
        }
        new ModuleRegistry(modules).attach(List.of(new PureLanguageExtension()));
    ...
    private static void execute(String[] args) throws Exception
    {
        ...
        // Register PDB modules — each one carries its identity in its embedded manifest —
        // then check every declared dependency was loaded.
        ModuleRegistry registry = new ModuleRegistry();
        List<PdbModule> modules = new ArrayList<>();
        for (String pdbPath : pdbPaths)
        {
            PdbModule module = PdbModule.open(Path.of(pdbPath));
        ...
        // The registry is the resolver: every loaded PDB is in scope. Necessary for
